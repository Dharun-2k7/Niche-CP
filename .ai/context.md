...
*   (2026-07-30) Sandbox DooD Fix: Fixed an issue where custom test case executions (via the "Run Code" button) were failing with `failed to start container: exit status 1`. This was caused because the API container (which handles the synchronous `POST /api/execute` endpoint) lacked access to the Docker socket. Added `/var/run/docker.sock` and `/tmp` cache volume mounts to the `api` service in `docker-compose.prod.yml`.
