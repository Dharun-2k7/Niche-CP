...
func GetSolvedProblems(c *gin.Context) {
	...
	rows, err := db.DB.Query(`SELECT DISTINCT problem_id FROM user_problem_status WHERE user_id = $1 AND status = 'ACCEPTED'`, userID)
	if err != nil {
...
func UpdateProfile(c *gin.Context) {
	...
	if req.Email != currentEmail {
		...
		body := fmt.Sprintf("Hello %s,\n\nYou requested to change your email address. To proceed, please use this OTP: %s\n\nThis code expires in 10 minutes.", req.Name, otp)
		go mailer.SendEmail(currentEmail, "NicheCP Email Update Verification", body)

		c.JSON(http.StatusOK, gin.H{"message": "Profile name updated. OTP sent to your current email for verification.", "require_otp": true, "step": "verify_existing"})
		return
	}
...
func VerifyExistingEmailOTP(c *gin.Context) {
	...
	// Step 2: Generate and send OTP to the NEW email
	newOtp := generateOTP()
	newKey := fmt.Sprintf("email_update_new_otp:%d", userID)

	err = db.RedisClient.Set(context.Background(), newKey, newEmail+":"+newOtp, 10*time.Minute).Err()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to generate OTP for new email"})
		return
	}

	body := fmt.Sprintf("Hello,\n\nPlease verify your new NicheCP email address using this OTP: %s\n\nThis code expires in 10 minutes.", newOtp)
	go mailer.SendEmail(newEmail, "NicheCP New Email Verification", body)

	c.JSON(http.StatusOK, gin.H{"message": "Existing email verified. OTP sent to your new email.", "step": "verify_new"})
}

func VerifyNewEmailOTP(c *gin.Context) {
	var req VerifyEmailUpdateRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	userID, _ := c.Get("user_id")
	key := fmt.Sprintf("email_update_new_otp:%d", userID)
	...
	// Update DB and set last_email_change
	_, err = db.DB.Exec(`UPDATE users SET email = $1, pending_email = NULL, last_email_change = CURRENT_TIMESTAMP WHERE id = $2`, newEmail, userID)
	if err != nil {
	...
	body := "Your NicheCP email address has been updated successfully."
	go mailer.SendEmail(newEmail, "NicheCP Email Updated", body)
